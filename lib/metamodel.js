 ...
 * * Specialization/Generalization handling:
 * - When a real element matches an MM element, it also matches all ancestors (generalizations) in the hierarchy
 * - Relationships defined on generalized elements are inherited by specialized elements
 * - Cycles in specialization relationships are detected and handled to prevent infinite loops
 * * (c) Y. Moldawski, 2026
 */
...
/**
 * Gets all ancestor elements in the specialization hierarchy for a given MM element
 * Returns array of ancestor element IDs (including the element itself)
 * Handles cycles by tracking visited elements
 * @param {string} elementId - The MM element ID
 * @param {Object} metaModel - The meta-model
 * @returns {Array} Array of ancestor element IDs
 */
function getElementAncestors(elementId, metaModel) {
  var ancestors = [];
  var visited = {};
  ...
  function collectAncestors(id) {
    ...
    // Follow the specialization parent
    var parentId = metaModel.specializationParents[id];
    if (parentId) {
      collectAncestors(parentId);
    }
  }

  collectAncestors(elementId);
  return ancestors;
}
 ...
 * Includes all ancestors in the specialization hierarchy
 */
function findMetaModelMatches(element, metaModel) {
  ...
  if (specialization) {
    ...
    if (candidates) {
      directMatches = directMatches.concat(candidates);
    }
  } else {
  ...
  // For each direct match, also include all its ancestors (generalizations)
  var allMatches = [];
  var seenIds = {};

  directMatches.forEach(function (mmElement) {
    var ancestors = getElementAncestors(mmElement.id, metaModel);
    ancestors.forEach(function (ancestorId) {
      if (!seenIds[ancestorId]) {
        seenIds[ancestorId] = true;
        allMatches.push(metaModel.elementIds[ancestorId]);
      }
    });
  });
...
/**
 * Compares two relationship types, treating aggregation and composition as equivalent
 * @param {string} type1 - First relationship type
 * @param {string} type2 - Second relationship type
 * @returns {boolean} True if types match (considering aggregation/composition equivalence)
 */
function compareRelType(type1, type2) {
  // Direct match
  if (type1 === type2) {
    return true;
  }
  ...
  if (
    (type1 === "aggregation-relationship" || type1 === "composition-relationship") &&
    (type2 === "aggregation-relationship" || type2 === "composition-relationship")
  ) {
    return true;
  }
...
/**
 * Finds matching meta-model relationships for a real relationship
 * Returns array of matching MM relationships (can be empty)
 * Checks both the relationship type/specialization AND that source/target elements match
 */
function findMetaModelRelationshipMatches(relationship, metaModel) {
  if (!metaModel) {
    return [];
  }

  var matches = [];
  var specialization = relationship.specialization;
  var type = relationship.type;

  // Get matches for source and target elements
  var sourceMatches = findMetaModelMatches(relationship.source, metaModel);
  var targetMatches = findMetaModelMatches(relationship.target, metaModel);

  // If either source or target has no matches in metamodel, no relationship can match
  if (sourceMatches.length === 0 || targetMatches.length === 0) {
    return [];
  }

  // Match relationships from meta-model
  metaModel.relationships.forEach(function (mmRel) {
    var relationshipTypeMatches = false;

    // Check if relationship type/specialization matches
    if (specialization) {
      // Match by specialization (if relationship has one)
      relationshipTypeMatches = mmRel.specialization === specialization;
    } else {
      // Match by type, but only if MM relationship also has no specialization
      relationshipTypeMatches = compareRelType(mmRel.type, type) && !mmRel.specialization;
    }

    if (relationshipTypeMatches) {
      // Check if source and target elements match
      var sourceElementMatches = false;
      var targetElementMatches = false;

      // Check if any of the source matches equals the MM relationship's source
      for (var i = 0; i < sourceMatches.length; i++) {
        if (sourceMatches[i].id === mmRel.source.id) {
          sourceElementMatches = true;
          break;
        }
      }

      // Check if any of the target matches equals the MM relationship's target
      for (var j = 0; j < targetMatches.length; j++) {
        if (targetMatches[j].id === mmRel.target.id) {
          targetElementMatches = true;
          break;
        }
      }

      // Only add if both source and target match
      if (sourceElementMatches && targetElementMatches) {
        matches.push(mmRel);
      }
    }
  });
...
function matchesRelationship(realRel, mmRel, debugEnabled) {
  ...
  // Compare relationship types (handles aggregation/composition equivalence)
  var typeMatches = compareRelType(realRel.type, mmRel.type);
  if (debugEnabled) {
...
/**
 * Gets the visual formatting properties from a meta-model element's first visual representation
 * Returns an object with formatting properties or null if no visual found
 * @param {Element} mmElement - The meta-model element
 * @param {View} metaView - The view containing the meta-model
 * @returns {Object|null} Formatting properties: { fontSize, fontName, fontStyle, textAlignment, textPosition, labelExpression }
 */
function getMetaModelFormatting(mmElement, metaView) {
  // Find the first visual representation of this element on the meta-model view
  var visual = null;

  $(metaView)
    .find("element")
    .each(function (v) {
      if (v.concept.id === mmElement.id) {
        visual = v;
        return false; // break
      }
    });
...
/**
 * Gets the visual formatting properties from a meta-model relationship's first visual representation
 * Returns an object with formatting properties or null if no visual found
 * @param {Relationship} mmRelationship - The meta-model relationship
 * @param {View} metaView - The view containing the meta-model
 * @returns {Object|null} Formatting properties: { fontSize, fontName, fontStyle, textAlignment, textPosition, labelExpression }
 */
function getMetaModelRelationshipFormatting(mmRelationship, metaView) {
  // Find the first visual representation of this relationship on the meta-model view
  var visual = null;

  $(metaView)
    .find("relationship")
    .each(function (v) {
      if (v.concept.id === mmRelationship.id) {
        visual = v;
        return false; // break
      }
    });
  ...
  // Extract formatting properties (same as for elements)
  var formatting = {
    fontSize: visual.fontSize,
    fontName: visual.font,
    fontStyle: visual.fontStyle,
...
/**
 * Applies formatting to a visual element
 * @param {VisualElement} visualElement - The visual element to format
 * @param {Object} formatting - Formatting properties to apply
 */
function applyFormatting(visualElement, formatting) {
  if (!formatting) {
    return;
  }

  if (formatting.fontSize !== undefined && formatting.fontSize !== null) {
    visualElement.fontSize = formatting.fontSize;
  }

  if (formatting.fontName !== undefined && formatting.fontName !== null) {
    visualElement.font = formatting.fontName;
  }

  if (formatting.fontStyle !== undefined && formatting.fontStyle !== null) {
    visualElement.fontStyle = formatting.fontStyle;
  }

  if (formatting.textAlignment !== undefined && formatting.textAlignment !== null) {
    visualElement.textAlignment = formatting.textAlignment;
  }

  if (formatting.textPosition !== undefined && formatting.textPosition !== null) {
    visualElement.textPosition = formatting.textPosition;
  }

  if (formatting.labelExpression !== undefined && formatting.labelExpression !== null) {
    visualElement.labelExpression = formatting.labelExpression;
  }
}
...
module.exports = {
  ...
  findMetaModelRelationshipMatches: findMetaModelRelationshipMatches,
  isRelationshipAllowed: isRelationshipAllowed,
  getMetaModelFormatting: getMetaModelFormatting,
  getMetaModelRelationshipFormatting: getMetaModelRelationshipFormatting,
